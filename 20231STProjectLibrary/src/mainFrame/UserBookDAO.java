...
public class UserBookDAO extends MemberDAO {
	...
	public ArrayList<BookVo> list1 (String id) {
		ArrayList<BookVo> list = new ArrayList<BookVo>();
		//
		try {
			connDB();
	...
	public String getDeleted(String id) {
		String inp = null;

		try {
			connDB();
			String query = "SELECT *\r\n" + "FROM G_USER g,RENTAL r \r\n"
					+ "WHERE g.ID = r.RENTAL_USER (+) AND r.RENTAL_DATE IS NOT NULL and r.RETURN_DATE IS NULL";
			if (id != null) {
				query += " AND g.id='" + id + "'";
			}
			System.out.println("SQL : " + query);
